...
int main()
{
    ...
    while (1)
    {
        // Display the slack time of each process at the current time
        printf("Current Time: %d\n", current_time);
        printf("Process | Slack Time\n");
        for (int i = 0; i < num_processes; i++)
        {
            if (processes[i].arrival_time <= current_time && processes[i].burst_time > 0)
            {
                int slack_time = processes[i].deadline - current_time - processes[i].burst_time;
                printf("   %d    |     %d\n", processes[i].id, slack_time);
            }
        }
        printf("\n");

        // Find the process with the least slack time

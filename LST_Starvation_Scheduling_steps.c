...
int main()
{
    ...
    while (!is_all_completed(processes, num_processes))
    {
        ...
        // Display slack time, starvation time, and priority of each process
        printf("Time %d:\n", current_time);
        for (int i = 0; i < num_processes; i++)
        {
            if (processes[i].arrival_time <= current_time && processes[i].burst_time > 0)
            {
                printf("Process %s - Slack Time: %d, Starvation Time: %d, Priority: %.2f\n",
                       processes[i].id, processes[i].slack_time, processes[i].starvation_time, processes[i].priority);
            }
        }
        ...
        printf(" %d %s %d,\n", current_time - 1, processes[process_index].id, current_time);
